...
export default function JoinTab() {
  ...
  const coolSmsSend = async(e) => {
    console.log('coolsmsSend발송 함수 호출,company member joinseelct.js요소 고유 email,name,phone,cernum값등 조회 현재의 값:',name,phone,cernum);
    ...
    if(check_id){
      switch(check_id){
        ...
        case 'check2':
          ...
          document.getElementById('CompanyJoinAgree_move').style.display='block';
          document.getElementById('CompanyJoinInfo_move').style.display='none';
        break;
      }
    }else{
   ...
   //다음단계 버튼 누른경우에 넘길지 말지 여부
    const nextStep = (e) => {
      ...
      if(checkVaildate2()){
        ...
        //사업자 대표인지 여부는 어차피 관리자이기에 추가 분기는 없어도 되어보인다. 일단.
      }else{
        setActive2(false);
